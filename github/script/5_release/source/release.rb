...
def get_level(cur_role = 'job00a')
	if($flag == 0) then
		...
		cur_level = `cd /nas/home/quwang/c/proj/ssh/ ;./get_level_from_p05 \"su - xcai1 -c \\"cd /nas/utl/presidio;cap checkup:code ROLES=#{cur_role}\\"\"  srwp05mgt001 2>&1 |grep '\\[out'|awk '{print $5}'|awk -F\/ '{print $5}'|sort|uniq`
	 	puts cur_level
		if cur_level == ""
