...
cap checkup:health  ROLES=<%= "#{$roles_sjrp.chomp}" %> 2>&1 | grep out | sort

cap checkup:package  ROLES=<%= "#{$roles_sjrp.chomp}" %> 2>&1 | grep out | sort

cap checkup:code  ROLES=<%= "#{$roles_sjrp.chomp}" %> 2>&1 | grep out | sort

cap backup:code  ROLES=<%= "#{$roles_sjrp.chomp}" %> 
...
STOP HTTPD and JBOSS on pool A
[COMMAND] 
cap stop:httpd  ROLES=<%= "#{$roles_a.chomp}" %>;

cap stop:jboss  ROLES=<%= "#{$roles_a.chomp}" %>;

Deploy package on pool A
[COMMAND]
cap -s build_label=<%= "#{$level.chomp}" %> deploy:package ROLES=<%= "#{$roles_a.chomp}" %>;



[DOING] Start HTTPD and JBOSS on pool A
[COMMAND]
cap start:httpd  ROLES=<%= "#{$roles_a.chomp}" %>
...
SWAP Pool A/B
[COMMAND]
cap inpool_in ROLES=<%= "#{$roles_a.chomp}" %>;cap inpool_out ROLES=<%= "#{$roles_b.chomp}" %>;
...
cap inpool_in ROLES=<%= "#{$roles_b.chomp}" %>
<% else %>
...
cap inpool_in ROLES=<%= "#{$roles_a.chomp}" %>,cfs01a
<% else %>
cap inpool_in ROLES=<%= "#{$roles_a.chomp}" %>
